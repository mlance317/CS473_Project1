...
//Create and return a texture using the stb_image.h functions.
unsigned int GetTexture (std::string fileName, int wrapS=GL_REPEAT, int wrapT=GL_REPEAT);
...
//Creates and returns a BasicShape circle given a VAO, a float radius, the number of points to render, and a location (centered)
BasicShape GetTextureCircle (VAOStruct vao, float radius=1.0, int points= 40, glm::vec3 location = glm::vec3(0.0));
...
//Creates and returns a BasicShape rectangle given a VAO, a point to start from, a width (x) and height (y).
BasicShape GetTextureRectangle (VAOStruct vao, glm::vec3 location = glm::vec3(0.0), float width = 1.0, float height = 1.0);


//Creates a ship hull placed at the provided lower_left point, width, and height.
BasicShape GetHull (VAOStruct vao, glm::vec3 lower_left = glm::vec3(0.0),float width=1.0,float height=1.0);
