...
int main () {
    ...
    //vector containing each shape of the rocket
    std::vector<BasicShape*> shapes {&left_cannon,&right_cannon,&triangle,
                                     &hull,&windshield};

    //vector containing the color of each shape, in the same order
    //  as the shape vector above
    std::vector<glm::vec4> colors {glm::vec4(0.7,0.3,0.3,1.0), //left cannon
                                   glm::vec4(0.7,0.3,0.3,1.0), //right cannon
                                   glm::vec4(0.5,0.5,0.6,1.0), //triangle
                                   glm::vec4(0.5,0.5,0.6,1.0), //hull
                                   glm::vec4(0.4,0.4,0.9,1.0), //windshield
                                   };
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        for (int i = 0; i < shapes.size(); i++) {
            shader.setVec4("set_color",colors[i]);
            shapes[i]->Draw();
            shader.setVec4("set_color",glm::vec4(0.0,0.0,0.0,1.0));
            shapes[i]->DrawEBO();
        }
